...
namespace RpaLib.ProcessAutomation
{
    ...
    public static class Rpa
    {
        ...
        public static void TraceRedirections(string filePath)
        {
            Trace.AutoFlush = true;
            StartConsoleTracer();
            StartTextWriterTracer(filePath);
        }

        public static void StartConsoleTracer()
        {
            var stdOutTracer = new ConsoleTraceListener();
            var stdErrTracer = new ConsoleTraceListener(true);

            stdOutTracer.Name = "stdOutTracer";
            stdErrTracer.Name = "stdErrTracer";

            Trace.Listeners.Add(stdOutTracer);
            //Trace.Listeners.Add(stdErrTracer);
        }

        public static void StartTextWriterTracer(string filePath)
        {
            var textWritterTraceListener = new TextWriterTraceListener(File.Open(filePath, FileMode.Append));
            textWritterTraceListener.Name = "stdOutReader";
            Trace.Listeners.Add(textWritterTraceListener);
        }
        ...
        public static dynamic Yaml2Dict(string path)
        {
            ...
            dynamic yamlAsDict = deserializer.Deserialize<dynamic>(yamlText);
