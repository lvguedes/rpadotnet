...
namespace DocuSign.CodeExamples.Authentication
{
    ...
    public static class JWTAuth
    {
        ...
        public static OAuthToken AuthenticateWithJWT(string api, string clientId, string impersonatedUserId, string authServer, byte[] privateKeyBytes)
        {
            ...
            var apiType = Enum.Parse(typeof(ExamplesAPIType), api);
            var scopes = new List<string>
                {
                    "signature",
                    "impersonation",
                };
            if (apiType.Equals(ExamplesAPIType.Rooms))
            {
            ...
            if (apiType.Equals(ExamplesAPIType.Click))
            {
                scopes.AddRange(new List<string>
                {
                    "click.manage",
            ...
            if (apiType.Equals(ExamplesAPIType.Monitor))
            {
                scopes.AddRange(new List<string>
                {
                    "signature",
            ...
            if (apiType.Equals(ExamplesAPIType.Admin))
            {
                scopes.AddRange(new List<string>
                {
                    "user_read",
