...
namespace RpaLib.SAP.Legacy
{
    public class SAPGui
    {
        ...
        public static string GetColumnTitles(string id, GuiGridView gridView, GuiSession session, string Column)
        {
            ...
            return Convert.ToString((gridView.GetColumnTitles(Column) as GuiCollection).Item(0));
        }
        ...
        public static GuiSession GetAnotherSession(int sessionIndex)
        {
            GuiConnection connection = SAPGui.GetConnection();
            GuiSession session = connection.Children.ElementAt(sessionIndex) as GuiSession;
            return session;
        }
        ...
        public static GuiConnection CreateConnection(string description)
        {
            CSapROTWrapper sapROTWrapper = new CSapROTWrapper();
            object SapGuilRot = sapROTWrapper.GetROTEntry("SAPGUI");
        ...
        public static GuiConnection CreateConnection(string description, string user, string password)
        {
            CSapROTWrapper sapROTWrapper = new CSapROTWrapper();
            object SapGuilRot = sapROTWrapper.GetROTEntry("SAPGUI");
            object engine = SapGuilRot.GetType().InvokeMember("GetSCriptingEngine", System.Reflection.BindingFlags.InvokeMethod,
            ...
            if (app.Connections.Count == 0)
            {
                ...
                GuiStatusbar sapBar = (GuiStatusbar)session.FindById("/app/con[0]/ses[0]/wnd[0]/sbar");
                ...
                //GuiTextField mand = (GuiTextField)session.FindById("wnd[0]/usr/txtRSYST-MANDT");

                //name.Text = user;

                SAPGui.SetText("wnd[0]/usr/txtRSYST-BNAME", null, session, user);

                SAPGui.SetText("wnd[0]/usr/pwdRSYST-BCODE", null, session, password);
                //pass.Text = password;
                //lang.Text = language;
                //mand.Text = mandante;
